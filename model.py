...
from torch_geometric.nn import GATConv, TransformerConv, GATv2Conv, global_mean_pool, global_add_pool
import numpy as np
...
class Net_coor(torch.nn.Module):
    def __init__(self, num_features, args):
        ...
        # self.conv1 = GATConv(num_features, args.d_graph_layer, edge_dim = args.edge_dim)
        self.conv1 = GATv2Conv(num_features, args.d_graph_layer, edge_dim = args.edge_dim)
        ...
        # self.convs = torch.nn.ModuleList([GATConv(args.d_graph_layer, args.d_graph_layer, edge_dim = args.edge_dim) for _ in range(args.n_graph_layer)])
        self.convs = torch.nn.ModuleList([GATv2Conv(args.d_graph_layer, args.d_graph_layer, edge_dim = args.edge_dim) for _ in range(args.n_graph_layer)])
        # self.convs = torch.nn.ModuleList([torch.nn.Linear(args.d_graph_layer, args.d_graph_layer) for _ in range(args.n_graph_layer)])

        # self.conv2 = TransformerConv(args.d_graph_layer, 3, edge_dim = args.edge_dim)
        # self.conv2 = GATConv(args.d_graph_layer, 3, edge_dim = args.edge_dim)
        # self.conv2 = GATv2Conv(args.d_graph_layer, 3, edge_dim = args.edge_dim)
        # self.conv2 = torch.nn.Linear(args.d_graph_layer, 3)

        self.ln = torch.nn.LayerNorm(args.d_graph_layer)
        self.fc = torch.nn.Linear(args.d_graph_layer, 3)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x, w = self.conv1(x, edge_index, edge_attr, return_attention_weights=True)
        edges = w[0]
        weight = w[1]
        ...
        # 赋予边权重到节点上
        node_weights = torch.zeros(x.size(0), dtype=edge_weight.dtype, device=x.device)

        # 使用scatter_add_来累加边权重到节点上
        node_weights.scatter_add_(0, edge[0], edge_weight.squeeze())
        node_weights.scatter_add_(0, edge[1], edge_weight.squeeze())
