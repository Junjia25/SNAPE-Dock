...
parser.add_argument("--loss_curve",help="loss curve file name", type=str, default="loss_curve")
...
print(f"train_loader_dataset: {train_loader.dataset}")
...
elif args.model_type == 'Net_coor': # use this
...
def train():
    ...
    for data in train_loader:
        with torch.cuda.amp.autocast():
            ...
            optimizer.zero_grad()
            if args.flexible:
                if args.model_type != 'Net_coor_cent':  # 运行
                    # pred = model(data.x, data.edge_index, data.dist)[data.flexible_idx.bool()]
                     
                    # 获取模型的输出
                    pred, node_weights = model(data.x, data.edge_index, data.dist)
                    # 使用data.flexible_idx.bool()对pred和node_weights进行索引
                    pred = pred[data.flexible_idx.bool()]
                    # node_weights = node_weights[data.flexible_idx.bool()]
                ...
                # print("len_pred_else: ", len(pred))
                # print(f"pred_else: {pred}")
                loss = loss_op(pred, data.y)
    ...
    # print(f"len_pred_all: ", len(pred))
    # print(f"pred_all: {pred}")
...
def test(loader, epoch):
    ...
    for data in loader:
        ...
        if args.flexible:
            if args.model_type != 'Net_coor_cent': # 执行
                ...
                # out = out.cpu().detach().numpy()[data.flexible_idx.bool()]
                node_weights = node_weights.cpu().detach().numpy()[data.flexible_idx.bool()]
    ...
    return total_loss / pose_idx, avg_rmsd_per_pdb, avg_rmsd_per_pdb_in, all_rmsds, rmsd_per_pdb, num_pose_per_pdb, pdbs, out_atom_list, node_weights_list
...
for epoch in range(args.start_epoch, args.start_epoch + args.epoch):
    ...
    loss, rmsd, rmsd_in, all_rmsds, rmsd_per_pdb, num_pose_per_pdb, pdbs, out_atom_list, node_weights_list = test(test_loader, epoch)
    test_losses.append(loss)
    # loss, rmsd, rmsd_in, all_rmsds = test(test_loader, epoch)
    ...
    if epoch > 3 and (min_loss > loss or epoch % 5 == 0):
        saved_model_dir = os.path.join(args.model_dir, f'model_{epoch}.pt')
        torch.save(model, saved_model_dir)
        os.system(f'chmod 777 {saved_model_dir}')
        print(f"save model at epoch {epoch}, rmsd of {rmsd*100} !!!!!!!!")
    ...
    os.system(f'chmod 777 {args.output}')
print(f"\nBest model at epoch {best_epoch}, loss is {min_loss}")
...
plt.plot(epochs, train_losses, color='b', linestyle='-', linewidth=2, label='training loss')
plt.plot(epochs, test_losses, color='r', linestyle='-', linewidth=2, label='test loss')
# plt.axvline(x=32, color='k', linestyle='--')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.show()
plt.savefig(args.loss_curve + '.png')